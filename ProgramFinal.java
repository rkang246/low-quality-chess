...
public class ProgramFinal {
    ...
    /*public static char[][] board = {
//the stalemate and checkmate cause en passant issue, f2 f3, then e7 e5
            {'…', '…', '…', '…', '…', '♝', '♞', '♜'},
            {'…', '…', '…', '…', '♟', '…', '♟', '♛'},
            {'…', '…', '…', '…', '♕', '♟', '♚', '♜'},
            {'…', '…', '…', '…', '…', '…', '…', '♟'},
    ...
    /*public static int blackX = 2;
    public static int blackY = 6;
    public static int whiteX = 7;
    public static int whiteY = 4;*///
    ...
    public static void main(String[] args) {
        ...
        while (true) { //Turn Loop: one black, one white
            ...
            if (stalemate("B")) {
                System.out.println("The game has tied. Black is in stalemate.");
                break gameLoop;
            }
            ...
            if (stalemate("B")) {
                printBoard();
                System.out.println("The game has tied. White is in stalemate.");
                break gameLoop;
            }
            if (check("W")) {
                whiteCheck = true;
                ...
                for (int j = 0; j < 8; j++) {
                    if (isWhitePiece(board[i][j])) {
                        for (int x = 0; x < 8; x++) {
                            yLoop:
                            for (int y = 0; y < 8; y++) {
                                String mv = numToLetter(j) + "" + (8 - i) + " " + numToLetter(y) + (8 - x);
                                if (validInput(mv, player) && possibleMove(mv, player)) {
                                    char[][] tempBoard = copyBoard();
                    
                                    int xi = 8 - Character.getNumericValue(mv.charAt(1));
                                    int yi = letterToNum(mv.charAt(0));
                                    int xf = 8 - Character.getNumericValue(mv.charAt(4));
                                    int yf = letterToNum(mv.charAt(3));
